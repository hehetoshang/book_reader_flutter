看起来您使用的 `pdfrx` 版本与我的建议有API差异。让我根据您的错误信息和实际代码来修正：

## 主要问题

1. **`pagesCount` 属性不存在**：在您的 `pdfrx` 版本中，可能使用 `pageCount` 或 `length` 而不是 `pagesCount`
2. **`onDocumentLoaded` 参数不存在**：在 `PdfViewerParams` 中没有这个回调
3. **`bottomToolbar` 参数不存在**：`PdfViewer.file` 没有这个命名参数
    ...
    try {
      ...
      try {
        ...
        if (document is PdfDocument) {
          // 根据pdfrx文档，可能是pageCount属性
          // 使用反射或尝试常见属性名
          pageCount = _getPageCount(document);
        }
      } catch (e) {
...
直接使用您原来的代码，但在 `onPageChanged` 回调中动态更新总页数：
...
params: PdfViewerParams(
  onPageChanged: (pageNumber) {
    setState(() {
      ...
      // 如果总页数未知，尝试通过其他方式获取
      if (_totalPages <= 1) {
        // 可能可以通过 controller 获取文档信息
        _totalPages = _controller.pageCount ?? 100; // 假设值
      }
    });
    _updateProgress();
  },
...
1. 检查您的 `pdfrx` 版本：`flutter pub deps | findstr pdfrx`
2. 查看官方文档对应版本的API：https://pub.dev/packages/pdfrx
3. 考虑升级到最新版本：`flutter pub upgrade pdfrx`